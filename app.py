...
from langchain_community.document_loaders import PyPDFLoader # Assuming you might want PDF back
...
def build_rag_pipeline():
    ...
    print(f"No existing database found. Building a new one from documents in '{PROFILE_DIR}'...")
    if not os.path.exists(PROFILE_DIR):
        print(f"Error: Profile directory '{PROFILE_DIR}' not found. Please create it and add your JSON files.")
        return None
    ...
    if not documents:
        print(f"\n--- ERROR ---")
        print(f"No valid JSON documents were found in the '{PROFILE_DIR}' directory.")
        print(f"---------------")
        exit()
    ...
    print(f"Created {len(chunks)} document chunks from {len(documents)} file(s).")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--rebuild-db', action='store_true', help="Force deletion and rebuilding of the vector database.")
    args = parser.parse_args()

    if args.rebuild_db and os.path.exists(DB_DIR):
        print(f"Rebuild flag detected. Deleting existing database at '{DB_DIR}'...")
